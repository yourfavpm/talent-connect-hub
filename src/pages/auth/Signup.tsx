...
const Signup = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        // Assign roles based on portal
        const role = isTalent ? "talent" : "client";
        const { error: roleError } = await supabase
          .from("user_roles")
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          .insert({ user_id: data.user.id, role } as any);
        
        if (roleError) console.error(`Failed to assign ${role} role:`, roleError);
  ...
  return (
    <div className="min-h-screen flex flex-col-reverse lg:flex-row bg-white font-inter overflow-x-hidden">
      ...
      <div className="lg:w-[40%] relative border-r border-slate-100 flex flex-col justify-between overflow-hidden text-white">
        {/* Background Image with Overlay */}
        <div className="absolute inset-0 z-0 transition-all duration-700">
          <img 
            src={isTalent ? "/images/auth/talent-side.jpg" : "/images/auth/client-side.jpg"}
            alt="Background"
        ...
        <div className="relative z-10 p-8 lg:p-16 flex flex-col justify-between h-full">
          ...
          <div className="mt-auto pt-10">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/30">
              The ecosystem for high-stakes execution
            </p>
          </div>
        </div>
      </div>
