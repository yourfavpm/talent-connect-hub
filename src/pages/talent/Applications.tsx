...
const statusConfig: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
    ...
    interview_requested: { label: "Interview Requested", color: "bg-amber-500/10 text-amber-600 border-amber-500/30", icon: <Timer className="h-3 w-3" /> },
    interview_scheduled: { label: "Interview Scheduled", color: "bg-amber-500/10 text-amber-600 border-amber-500/30", icon: <Calendar className="h-3 w-3" /> },
    offer_initiated: { label: "Offer Pending", color: "bg-purple-500/10 text-purple-600 border-purple-500/30", icon: <DollarSign className="h-3 w-3" /> },
    offer_sent: { label: "Offer Received", color: "bg-purple-500/10 text-purple-600 border-purple-500/30", icon: <DollarSign className="h-3 w-3" /> },
    rejected: { label: "Rejected", color: "bg-red-500/10 text-red-600 border-red-500/30", icon: <XCircle className="h-3 w-3" /> },
    hired: { label: "Hired", color: "bg-green-600/10 text-green-700 border-green-600/30", icon: <CheckCircle className="h-3 w-3" /> },
                    ...
                    {filteredApplications.map((application) => {
                        ...
                        return (
                            <Card
                                ...
                                onClick={() => navigate(`/talent/jobs/${application.job_id}`)}
                            >
                                                ...
                                                <div className="space-y-2 flex-1">
                                                    <div className="flex items-start justify-between">
                                                        ...
                                                        <div className="flex flex-col items-end gap-2">
                                                            ...
                                                            {(application.status === 'offer_sent' || application.status === 'offer_initiated') && (
                                                                <Button size="sm" className="bg-purple-600 hover:bg-purple-700 text-white h-7 text-xs" asChild>
                                                                    <Link to="/talent/offers">View Offer</Link>
                                                                </Button>
                                                            )}
                                                        </div>
                                                    </div>
