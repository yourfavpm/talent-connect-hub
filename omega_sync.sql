...
BEGIN;
DO $$ BEGIN
    -- 1. Clean Audit Logs & Admin Users
    ...
    -- 3. Clean talents user_id
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='talents' AND column_name='user_id' AND data_type='text') THEN
        ALTER TABLE public.talents ALTER COLUMN user_id TYPE UUID USING user_id::uuid;
    END IF;

    -- 4. Clean user_roles user_id
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_roles' AND column_name='user_id' AND data_type='text') THEN
        ALTER TABLE public.user_roles ALTER COLUMN user_id TYPE UUID USING user_id::uuid;
    END IF;

    -- 5. Clean support_tickets user_id
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='support_tickets' AND column_name='user_id' AND data_type='text') THEN
        ALTER TABLE public.support_tickets ALTER COLUMN user_id TYPE UUID USING user_id::uuid;
    END IF;

    -- 6. Clean profiles user_id
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='user_id' AND data_type='text') THEN
...
CREATE POLICY "Users can view their own enrollments" ON academy_enrollments
        FOR SELECT USING (auth.uid() = user_id::uuid OR public.is_admin(auth.uid()));
EXCEPTION WHEN duplicate_object THEN null; END $$;
...
FOR SELECT USING (auth.uid() = student_id::uuid OR public.is_admin(auth.uid()));
