...
DO $$ BEGIN
    ...
    DECLARE
        ...
        tables_to_fix TEXT[][] := ARRAY[
            ...
            ['talent_profiles', 'assigned_admin_id'],
            ['talent_profile_sections', 'user_id'],
            ['vetting_actions', 'user_id'],
            ['vetting_actions', 'admin_id'],
            ['hr_v2_hire_requests', 'client_user_id'],
            ['hr_v2_hire_requests', 'approved_by_admin_id'],
            ['hr_v2_applications', 'talent_user_id'],
            ['hr_v2_shortlists', 'talent_user_id'],
            ['hr_v2_interviews', 'talent_user_id'],
            ['hr_v2_interviews', 'client_user_id'],
            ['hr_v2_interviews', 'admin_id'],
            ['hr_v2_hires', 'talent_user_id'],
            ['hr_v2_hires', 'client_user_id'],
            ['hr_v2_hires', 'admin_id'],
            ['academy_enrollments', 'user_id'],
            ['submissions', 'student_id'],
            ['academy_certificates', 'student_id'],
            ['onboarding_records', 'user_id'],
            ['payouts', 'talent_id'],
            ['payouts', 'batch_id'],
            ['payout_batches', 'created_by'],
            ['payout_audit_log', 'admin_id'],
            ['talent_payment_methods', 'talent_id'],
            ['deduction_records', 'talent_id'],
            ['admin_roles', 'admin_id'],
            ['admin_permission_overrides', 'admin_id'],
            ['talent_profile_steps', 'talent_id'],
            ['talent_profile_reviews', 'talent_id'],
            ['talent_work_history', 'talent_id'],
            ['talent_education', 'talent_id'],
            ['talent_certifications', 'talent_id']
        ];
    BEGIN
...
USING (
  bucket_id = 'talent_documents' AND EXISTS (
    ...
    WHERE user_id::uuid = auth.uid() AND role IN ('super_admin', 'operations_admin', 'vetting_admin', 'talent_manager')
  )
);
...
CREATE POLICY "Talents view own payment methods" ON public.talent_payment_methods FOR SELECT USING (auth.uid()::uuid IN (SELECT user_id::uuid FROM public.talents WHERE id = talent_id));
DROP POLICY IF EXISTS "Talents manage own payment methods" ON public.talent_payment_methods;
CREATE POLICY "Talents manage own payment methods" ON public.talent_payment_methods FOR ALL USING (auth.uid()::uuid IN (SELECT user_id::uuid FROM public.talents WHERE id = talent_id));
DROP POLICY IF EXISTS "Talents view own payouts" ON public.payouts;
CREATE POLICY "Talents view own payouts" ON public.payouts FOR SELECT USING (auth.uid()::uuid IN (SELECT user_id::uuid FROM public.talents WHERE id = talent_id));
...
BEGIN
    ...
    IF (NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid')) THEN
        ...
        JOIN public.timesheets ts ON ts.contract_id::uuid = c.id::uuid
...
CREATE POLICY "Clients view own invoice_line_items" ON public.invoice_line_items FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.invoices i JOIN public.clients c ON i.client_id = c.id WHERE i.id = invoice_line_items.invoice_id AND c.user_id = auth.uid()::uuid)
);
...
CREATE POLICY "Clients view own invoice_payment_records" ON public.invoice_payment_records FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.invoices i JOIN public.clients c ON i.client_id = c.id WHERE i.id = invoice_payment_records.invoice_id AND c.user_id = auth.uid()::uuid)
);
...
FROM auth.users u
JOIN public.user_roles ur ON u.id::uuid = ur.user_id::uuid
...
BEGIN
    RETURN EXISTS (
        ...
        WHERE ar.admin_id = auth.uid()::uuid AND p.key = p_key
    ) OR EXISTS (
        SELECT 1 FROM public.admin_permission_overrides apo
        JOIN public.permissions p ON apo.permission_id = p.id
        WHERE apo.admin_id = auth.uid()::uuid AND p.key = p_key AND apo.allowed = true
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
DROP FUNCTION IF EXISTS public.get_admin_permissions(p_admin_id UUID) CASCADE;
...
BEGIN
    ...
    VALUES (auth.uid()::uuid, 'INVITE_ADMIN', 'team', v_admin_id);
...
BEGIN
    ...
    -- 3. Log the action
    INSERT INTO public.audit_logs (admin_id, action, module, target_id)
    VALUES (auth.uid()::uuid, 'UPDATE_ROLE', 'team', p_admin_id);
END;
$$;
...
BEGIN
    ...
    -- Log the action
    INSERT INTO public.audit_logs (admin_id, action, module, target_id)
    VALUES (auth.uid()::uuid, 'UPDATE_STATUS', 'team', p_admin_id);
END;
$$;
...
BEGIN
    ...
    -- Log the action
    INSERT INTO public.audit_logs (admin_id, action, module, target_id)
    VALUES (auth.uid()::uuid, 'UPSERT_OVERRIDE', 'team', p_admin_id);
END;
$$;
...
CREATE POLICY "Admins can view settings" ON public.organization_settings FOR SELECT USING (EXISTS (SELECT 1 FROM public.admin_users WHERE id = auth.uid()::uuid));
DROP POLICY IF EXISTS "Admins can view pricing" ON public.pricing_rules;
CREATE POLICY "Admins can view pricing" ON public.pricing_rules FOR SELECT USING (EXISTS (SELECT 1 FROM public.admin_users WHERE id = auth.uid()::uuid));
...
USING (EXISTS (SELECT 1 FROM public.talents WHERE talents.id = talent_profile_steps.talent_id AND talents.user_id = auth.uid()::uuid));
...
USING (EXISTS (SELECT 1 FROM public.profiles WHERE profiles.user_id = auth.uid()::uuid));
EXCEPTION WHEN duplicate_object THEN null;
END $$;
...
USING (EXISTS (SELECT 1 FROM public.talents WHERE talents.id = step_change_requests.talent_id AND talents.user_id = auth.uid()::uuid));
EXCEPTION WHEN duplicate_object THEN null;
END $$;
...
USING (EXISTS (SELECT 1 FROM public.talents WHERE talents.id = talent_profile_reviews.talent_id AND talents.user_id = auth.uid()::uuid));
EXCEPTION WHEN duplicate_object THEN null;
END $$;
...
WITH CHECK (EXISTS (SELECT 1 FROM public.talents WHERE talents.id = talent_profile_reviews.talent_id AND talents.user_id = auth.uid()::uuid));
EXCEPTION WHEN duplicate_object THEN null;
END $$;
...
CREATE POLICY "Talents can view their assigned manager profiles" ON public.profiles
        ...
        USING (
            EXISTS (
                ...
                WHERE talents.user_id = auth.uid()::uuid
...
USING (EXISTS (SELECT 1 FROM public.profiles WHERE profiles.user_id::uuid = auth.uid()::uuid));
...
CREATE OR REPLACE FUNCTION public.update_section_data(
    ...
    p_data JSONB,
    p_completion_percent INTEGER,
    p_user_id UUID DEFAULT auth.uid()::uuid
)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    WHERE user_id = COALESCE(p_user_id, auth.uid()::uuid);
END;
$$;

-- 8.2. submit_talent_onboarding
DROP FUNCTION IF EXISTS public.submit_talent_onboarding() CASCADE;
CREATE OR REPLACE FUNCTION public.submit_talent_onboarding(p_user_id UUID DEFAULT auth.uid()::uuid)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    SET 
        ...
        last_action_at = NOW(),
        updated_at = NOW(),
        assigned_admin_id = auth.uid()::uuid
    WHERE user_id = p_talent_user_id
    AND status = 'SUBMITTED';

    IF FOUND THEN
        INSERT INTO public.vetting_actions (user_id, admin_id, action_type, note)
        VALUES (p_talent_user_id, auth.uid()::uuid, 'START_REVIEW', 'Admin started reviewing profile');
    END IF;
END;
$$;
...
BEGIN
    ...
    VALUES (p_talent_user_id, auth.uid()::uuid, 'APPROVE_SECTION', p_section_key, 'Admin approved section');
END;
$$;
...
BEGIN
    ...
    SET 
        ...
        requested_by_admin_id = auth.uid()::uuid,
        updated_at = NOW()
    WHERE user_id = p_talent_user_id 
    AND section_key = p_section_key;
    ...
    VALUES (p_talent_user_id, auth.uid()::uuid, 'REQUEST_CHANGES', p_section_key, p_changes_note, jsonb_build_object('fields', p_fields));
...
BEGIN
    ...
    VALUES (p_talent_user_id, auth.uid()::uuid, 'MARK_VETTED', 'Admin finalized vetting and assigned level', jsonb_build_object('level', p_vetting_level));
...
CREATE OR REPLACE FUNCTION public.v2_recompute_progress(p_user_id UUID DEFAULT auth.uid())
RETURNS INT
LANGUAGE plpgsql
SECURITY DEFINER SET search_path = public
AS $$
...
BEGIN
    ...
    IF auth.uid()::uuid != p_user_id::uuid AND NOT public.is_admin(auth.uid()::uuid) THEN
...
LEFT JOIN public.talents t ON tp.user_id::uuid = t.user_id::uuid
LEFT JOIN public.profiles p ON tp.user_id::uuid = p.user_id::uuid
...
BEGIN
    ...
    FROM public.talents t
    LEFT JOIN public.profiles p ON t.user_id::uuid = p.user_id::uuid
...
USING (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = interview_requests.client_id AND c.user_id::uuid = auth.uid()::uuid));
...
USING (EXISTS (SELECT 1 FROM public.talents t WHERE t.id = interview_requests.talent_id AND t.user_id::uuid = auth.uid()::uuid));
